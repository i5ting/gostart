...
### Go tool is only compatible with code that resides in a workspace
...
### Go tool does not allow you to depend on specific versions of external packages
...
In a nut, don't try to emulate versioning with go tool, it just doesn't play well with it.

### When working with Go tool, use fully qualified imports and always build import paths from $GOPATH
...
Before you start any coding, you should pick a directory that will become your Go workspace. All your Go code will reside there. Set the `GOPATH` environment variable to the path to that directory in your .bashrc or similar file for the  you're using.
...
### How do I split my package into multiple files? ###
...
### How do I split my package into multiple subpackages? ###
...
### How do I create a package for others to use (i.e. a non-main package)? ###
...
### Where did output from running 'go build' and 'go install' go? ###
...
### Does it make sense to keep commands and packages in separate workspaces? ###
...
### Where can I find more informantion about learning Go? ###
...
### What is the current state of editor support for Go? ###
